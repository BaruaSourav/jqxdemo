...
class PHPExcel_Calculation_Financial {	
 ...
 //return strtotime($f);
	...
	public static function PRICE($settlement, $maturity, $rate, $yield, $redemption, $frequency, $basis=0) {
		...
		$ncd = self::COUPNCD($settlement, $maturity, $frequency, $basis);
		$pcd = self::COUPPCD($settlement, $maturity, $frequency, $basis);
		//$dif=$ncd-$pcd;
	     $e_Yf=self::YEARFRAC($pcd,$ncd,3);
		//$Yf=($Yf/2);
	     $a_Yf=self::YEARFRAC($pcd,$settlement,3);
		
		 $dsc_Yf=self::YEARFRAC($settlement,$ncd,3);
		
		
		
		$dsc = self::COUPDAYSNC($settlement, $maturity, $frequency, 1);
		$e = self::COUPDAYS($settlement, $maturity, $frequency, 1);
		$n = self::COUPNUM($settlement, $maturity, $frequency, $basis);
		$a = self::COUPDAYBS($settlement, $maturity, $frequency,1);
	  ...
	  //echo 'DSC='.$dsc.'  E='.$e.'  N='.$n.'  A='.$a ;
		// hosain
		$dsc=$dsc-($e_Yf);
		$e=($e)-($e_Yf);//number_format($e,0,'.',''); //floor
	 	$a=($a)-($e_Yf);//number_format($a,0,'.',''); //ceil
		
		/*echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'; 
		 echo 'DSC='.$dsc.'  E='.$e.'  N='.$n.'  A='.$a ;*/
		 
		$a=number_format($a,0,'.',''); //floor($a);//
		//$e=number_format($e,0,'.',''); //ceil
		//$dsc=number_format($dsc,0,'.',''); //ceil
