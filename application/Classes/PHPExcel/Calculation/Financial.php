...
class PHPExcel_Calculation_Financial {	
	...
	private static function _coupFirstPeriodDate($settlement, $maturity, $frequency, $next,$HOSAIN=NULL) {
		$months = 12 / $frequency;
		$result = PHPExcel_Shared_Date::ExcelToPHPObject($maturity);		
		$monthcount=$result->format('m');
		$daycount=$result->format('d');
		 
		$odd=1;
		if(($monthcount%2)==0){$odd=0;}		
		...
		while ($settlement < PHPExcel_Shared_Date::PHPToExcel($result)) {
			$result->modify('-'.$months.' months');	
			$new_odd=1;	
			if(($result->format('m')%2)==0){$new_odd=0;}
			if($odd!=$new_odd){
				$month=$result->format('m');					
				while($result->format('m')>=$month)
				{
					$result->modify('-1 day');
				}
			}
			...
			if($dd!=$daycount){
				if($mm=='08'){
					while($result->format('d')<$daycount)
					{
						$result->modify('+1 day');
					}
				}	
		...
		if($HOSAIN==1){return $result->format('d/m/Y');}
		else{return PHPExcel_Shared_Date::PHPToExcel($result);}
	...
	public static function COUPNCD($settlement, $maturity, $frequency, $basis=0, $HOSAIN=NULL) {
		$settlement	= PHPExcel_Calculation_Functions::flattenSingleValue($settlement);
		$maturity	= PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
		$frequency	= (int) PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
		$basis		= (is_null($basis))	? 0 :	(int) PHPExcel_Calculation_Functions::flattenSingleValue($basis);
		...
		return self::_coupFirstPeriodDate($settlement, $maturity, $frequency, True,$HOSAIN);
	}	//	function COUPNCD()
	...
	public static function COUPPCD($settlement, $maturity, $frequency, $basis=0, $HOSAIN=NULL) {
		$settlement	= PHPExcel_Calculation_Functions::flattenSingleValue($settlement);
		$maturity	= PHPExcel_Calculation_Functions::flattenSingleValue($maturity);
		$frequency	= (int) PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
		$basis		= (is_null($basis))	? 0 :	(int) PHPExcel_Calculation_Functions::flattenSingleValue($basis);
 $f=PHPExcel_Calculation_DateTime::_getDateValue($settlement);
 //return strtotime($f);
		if (is_string($settlement = PHPExcel_Calculation_DateTime::_getDateValue($settlement))) {
			return PHPExcel_Calculation_Functions::VALUE();
		}
		...
		return self::_coupFirstPeriodDate($settlement, $maturity, $frequency, False,$HOSAIN);
	}	//	function COUPPCD()
	...
	public static function PRICE($settlement, $maturity, $rate, $yield, $redemption, $frequency, $basis=0) {
		...
		if($n==1)
		{
			$discount=1 + $yield/$frequency * $dsc/$e;
			$discount=1 / $discount;
		
			$result=$redemption*$discount;
			$result+=100*$rate/$frequency*$discount;			
		}
		else{
	...
	public static function YIELD($settlement, $maturity, $rate, $price, $redemption, $frequency, $basis) {
		$settlement	=  PHPExcel_Calculation_Functions::flattenSingleValue($settlement); //40917;
		$maturity	= PHPExcel_Calculation_Functions::flattenSingleValue($maturity); //42586;
		$rate		= (float) PHPExcel_Calculation_Functions::flattenSingleValue($rate);
		$price		= (float) PHPExcel_Calculation_Functions::flattenSingleValue($price);
		$redemption	= (float) PHPExcel_Calculation_Functions::flattenSingleValue($redemption);
		$frequency	= (int) PHPExcel_Calculation_Functions::flattenSingleValue($frequency);
		...
		$n = self::COUPNUM($settlement, $maturity, $frequency, $basis);
		$cupdays = self::COUPDAYS($settlement, $maturity, $frequency, $basis);
		
		$C = $redemption*$rate*$cupdays/365; //coupon interest receipt
		
		
		
	   $Ia=($C-($price-$redemption)/$n)/(($price+$redemption)/2)*2;
		
		$PVa=self::PRICE($settlement, $maturity, $rate,$Ia, $redemption,$frequency, $basis);
		$f_ia=$PVa-$price;
		$f_drevative_Ia=((-$C *( 1- ($n*$Ia*pow((1+$Ia),-(1+$n)) ) +  pow((1+$Ia),-$n ) ))/pow($Ia,2))   -  ($n*$redemption*pow((1+$Ia),-($n+1)));
		
		$result=$Ia-($f_ia/$f_drevative_Ia);
	...
	/*public static function YIELD($settlement, $maturity, $rate, $price, $redemption, $frequency, $basis) {
		...
		//3510	
		//$dsc = self::COUPDAYSNC($settlement, $maturity, $frequency, $basis);
	//	$dsr = $daysBetweenSettlementAndMaturity;
		
		
		$dsr=self::COUPDAYSNC($settlement, $maturity, $frequency, $basis);
		$e=self::COUPDAYS($settlement, $maturity, $frequency, $basis);
		
		//$dsr=$e-self::COUPDAYBS($settlement, $maturity, $frequency, $basis);
		//=COUPDAYS(A2,B2,F2,G2)
//		 $e =180;// 
		//$n = self::COUPNUM($settlement, $maturity, $frequency, $basis);
		$a = self::COUPDAYBS($settlement, $maturity, $frequency, $basis);
		
	 //  $dsr=number_format($dsr,0,'.','');
	//$e=number_format($e,0,'.','');
		//$a=number_format($a,0,'.','');
		//echo $dsr=$e-$a.'<br />';
		
		$part1	= ($redemption/100) + ($rate / $frequency);
		$part2	= (($price/100) + (($a / $e) * ($rate / $frequency)));
		$part3	= ($frequency*$e) / $dsr ;
		
		$result = (($part1-$part2)/ $part2) * $part3;
	...
	public static function YEARFRAC($issue, $settlement,$basis) 
	{
		return PHPExcel_Calculation_DateTime::YEARFRAC($issue, $settlement, $basis);
	} 
	public static function get_date_diff($date_big, $date_small,$format=NULL) 
	{
		if($format=='/'){
		$date_big=self::get_date_format($date_big,'-');
		$date_small=self::get_date_format($date_small,'-');
		}
		$days =floor((strtotime($date_big)-strtotime($date_small))/86400);
		return $days;
	} 
	public static function get_date_format($date,$format) 
	{
		$date=PHPExcel_Calculation_Functions::flattenSingleValue($date);
		$date = PHPExcel_Calculation_DateTime::_getDateValue($date);
		$result = PHPExcel_Shared_Date::ExcelToPHPObject($date);
		if($format=='/'){
			return $result->format('d/m/Y');
		}else if($format=='-'){
	...
	public function get_maturity_dt($issudt,$tenor,$type)
	{
		if($type != 'T.Bond')
		{
			$maturitydt = date('Y-m-d', strtotime("+".$tenor." days", strtotime($issudt)));
		}
		elseif($type == 'T.Bond')
		{
			$maturitydt = date('Y-m-d', strtotime("+".$tenor." years", strtotime($issudt)));
		}
		return $maturitydt;
	}
	
	public function get_hp_interest($rate,$hp,$FV,$dtdiff)
	{
		$hpi = ($rate * $hp * $FV) / (100 * $dtdiff * 2);
		return $hpi;
	}
	
	public function b2_get_tenor_from_yearfraction($yearfraction,$dealtype)
	{
		if($dealtype == 'T.Bond')
		{
			if($yearfraction<=2){$tnor=2;}
			else if($yearfraction>2 && $yearfraction<=5){$tnor=5;}
			else if($yearfraction>5 && $yearfraction<=10){$tnor=10;}
			else if($yearfraction>10 && $yearfraction<=15){$tnor=15;}
			else if($yearfraction>15 && $yearfraction<=20){$tnor=20;}
			else {$tnor=20;}
		}
		else
		{
			if($yearfraction<=30){$tnor=30;}
			else if($yearfraction>30 && $yearfraction<=91){$tnor=91;}
			else if($yearfraction>91 && $yearfraction<=182){$tnor=182;}
			else if($yearfraction>182 && $yearfraction<=364){$tnor=364;}		
			else {$tnor=364;}
		}
		return $tnor;
	}
