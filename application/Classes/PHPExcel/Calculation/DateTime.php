		...
		if (((is_numeric($method)) && (!is_string($method))) || ($method == '')) {
			...
			switch($method) {
				...
				case 1	:
					...
					// change new code
					if ($years == 1) {					
						if (self::_isLeapYear($startYear)) {$leapDays += 1;}						
					}
					else if ($years == 2) {							
						if (self::_isLeapYear($startYear))
						{							
							if (self::MONTHOFYEAR($startDate)<3)
							{
								if($days>366){$leapDays=1/2;}else{$leapDays=1;}
							}
							else if (self::MONTHOFYEAR($startDate)>=3)
							{
								if($days>365){$leapDays=1/2;}else{$leapDays=0;}
							}							
						}
						else if (self::_isLeapYear($endYear))
						{							
							if (self::MONTHOFYEAR($endDate)==3 && self::DAYOFMONTH($endDate)==29)
							{
								if($days>366){$leapDays=1/2;}else{$leapDays=1;}
							}								
							else if (self::MONTHOFYEAR($endDate)>=3)
							{
								if($days>365){$leapDays=1/2;}else{$leapDays=0;}
							}
							else{	
